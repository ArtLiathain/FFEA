...
InteractionData::InteractionData(int rod_id_a, int rod_id_b, int elem_id_a,
    ...
    const float3  &shift,
    const float3  &r_a,
    const float3  &r_b, float eps, float rmin)
{
    rod_id_self = rod_id_a;
    rod_id_nbr = rod_id_b;
    elem_id_self = elem_id_a;
    ...
    vec3d(n){r_nbr[n] = r_b[n];}
    epsilon = eps;
    r_min = rmin;  // equilibrium distance, r_min = 2^(1/6) * sigma
    r_min_inv = 1 / r_min;
...
float vdw_energy_6_12(float r_inv, float eps, float r_min)
{
    float rr = r_min * r_inv;
    float rr3 = rr * rr * rr;
    float rr6 = rr3 * rr3;
    float rr12 = rr6 * rr6;
    return eps * (rr12 - 2 * rr6);
}

float vdw_force_6_12(float r_inv, float eps, float r_min)
{
    float rr = r_min * r_inv;
    float rr3 = rr * rr * rr;
    float rr6 = rr3 * rr3;
    float rr12 = rr6 * rr6;
    return -12 * eps * r_inv * (rr12 - rr6);
}
