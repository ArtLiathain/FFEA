...
void SSINT_matrix::init_ssint(const string &ssint_params_fname, const string &ssint_type, scalar ssint_cutoff) {
    ...
    // equilibrium distance: Rmin = 2^(1/6) * sigma
    // potential well depth: Emin = epsilon
    ...
    for (int i = 0; i < num_ssint_face_types; i++)
    {
        ...
        for (int j = 0; j < num_ssint_face_types; j++)
        {
            ...
            if (is_rod)
            {
                params[LJI(i, j)][head[0]] = params[LJI(i, j)][head[0]] / mesoDimensions::Energy ;  // epsilon [J]
                params[LJI(i, j)][head[1]] = params[LJI(i, j)][head[1]] / mesoDimensions::length;   // r_min [m]
            }
            else
            {
                params[LJI(i, j)][head[0]] = params[LJI(i, j)][head[0]] * mesoDimensions::area * mesoDimensions::area / mesoDimensions::Energy ;  // epsilon [J / m^4]
                params[LJI(i, j)][head[1]] = params[LJI(i, j)][head[1]] / mesoDimensions::length;  // r_min [m]
                params[LJI(i, j)][head[2]] = params[LJI(i, j)][head[2]] * mesoDimensions::area / mesoDimensions::Energy;  // J / m ^2
            }
	    }
    }
